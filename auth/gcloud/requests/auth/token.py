...
GCE_ENDPOINT_TOKEN = '{}/instance/service-accounts/default/token?recursive=true'.format(GCE_METADATA_BASE)
...
class Token:
    ...
    def __init__(self, service_file=None, session=None, scopes=None):
      # type: (Optional[str], requests.Session, List[str]) -> None
    ...
    def _refresh_authorized_user(self, timeout):
      ...
      return self.session.post(self.token_uri, data=payload, headers=REFRESH_HEADERS, timeout=timeout)

    def _refresh_gce_metadata(self, timeout):
      # type: (int) -> requests.Response
      return self.session.get(self.token_uri, headers=GCE_METADATA_HEADERS, timeout=timeout)
